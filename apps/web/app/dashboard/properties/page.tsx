...
import { archiveProperty } from "@/lib/api/properties";
...
export default function MyPropertiesPage() {
  ...
  const [archiving,setArchiving]=useState("");const [actionError,setActionError]=useState("");
  async function archive(id:string,title:string) {
    if(!window.confirm(`Archive “${title}”? This removes it from public search.`))return;
    setArchiving(id);setActionError("");
    try { await archiveProperty(id); if(data?.data.length===1&&page>1)setPage(page-1);else await retry(); }
    catch(cause){setActionError(cause instanceof ApiError?cause.message:"Could not archive this property.");}
